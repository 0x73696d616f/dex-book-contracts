...
library BokkyPooBahsRedBlackTreeLibrary {
    ...
    function next(Tree storage self, uint40 target) internal view returns (uint40 cursor) {
    ...
    function prev(Tree storage self, uint40 target) internal view returns (uint40 cursor) {
    ...
    function exists(Tree storage self, uint40 key) internal view returns (bool) {
    ...
    function getNode(Tree storage self, uint40 key)
        internal
        view
        returns (uint40 _returnKey, uint40 _parent, uint40 _left, uint40 _right, bool _red)
    {
    ...
    function insert(
        ...
        self.nodes[key] = Node({ parent: EMPTY, left: EMPTY, right: EMPTY, red: true, userData: userData });
        while (probe != EMPTY) {
            cursor = probe;
    ...
    function remove(Tree storage self, uint40 key, function(uint40, uint256) returns (bool) aggregate, uint256 data)
        internal
    {
    ...
    // destructive func
    function joinRight(Tree storage self, JoinParams memory params, function(uint40, uint256) returns (bool) aggregate)
        private
        returns (uint40, uint8)
    {
    ...
    // destructive func
    function joinLeft(Tree storage self, JoinParams memory params, function(uint40, uint256) returns (bool) aggregate)
        internal
        returns (uint40 resultKey)
    {
    ...
    function splitRight(
        ...
        if (lessThan(params.key, params.t, params.data)) {
            ...
            return join(
                self, r, params.t, self.nodes[params.t].right, aggregate, rBlackHeight, params.blackHeight, params.data
            );
        } else {
            subtreeRemoved(params.t, params.data);
            return splitRight(
    ...
    function treeMinimum(Tree storage self, uint40 key) private view returns (uint40) {
    ...
    function treeMaximum(Tree storage self, uint40 key) private view returns (uint40) {
    ...
    function rotateLeft(Tree storage self, uint40 key, function(uint40, uint256) returns (bool) aggregate, uint256 data)
        private
    {
        ...
        while (key != self.root && self.nodes[self.nodes[key].parent].red) {
            ...
            if (keyParent == self.nodes[self.nodes[keyParent].parent].left) {
                ...
                if (self.nodes[cursor].red) {
                    ...
                    self.nodes[keyParent].red = false;
                    self.nodes[self.nodes[keyParent].parent].red = true;
                    rotateRight(self, self.nodes[keyParent].parent, aggregate, data);
                }
            } else {
                ...
                if (self.nodes[cursor].red) {
                    ...
                    self.nodes[keyParent].red = false;
                    self.nodes[self.nodes[keyParent].parent].red = true;
                    rotateLeft(self, self.nodes[keyParent].parent, aggregate, data);
                }
            }
        }
        self.nodes[self.root].red = false;
    ...
    function replaceParent(Tree storage self, uint40 a, uint40 b) private {
